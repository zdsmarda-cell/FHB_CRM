...
// Inject the email function and background jobs before app.listen
const jobsCode = `
...
async function sendAssignmentEmail(hunterId: string, dealId: string, companyName: string, connection: any) {
  try {
    ...
    const link = \`\${appUrl}/deals/\${dealId}\`;
...
if (!content.includes('runHourlyJob')) {
  content = content.replace("  app.listen(PORT,", jobsCode);
}
// check if import nodemailer is already there
if (content.includes('import nodemailer')) {
   // Already imported? Wait, our injection has it. 
   // Actually, we can just replace 'import nodemailer' if it's placed inside the function block, it's fine for typescript/node if we use top level, but ESM needs it at the top.
}
